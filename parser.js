...
Parser.prototype.read_reg_sequence = function(str,pos) {
	var data = seq({
		...
		patterns: rep(seq({
				pattern: any( // паттерны
					reg_symbol.then((symbol,x)=>({type:'symbol',symbol,pos:x})),
		...
		(data)=>{
			if(!data.separator && data.patterns.length>0) {
				...
				data.handlers.concat(lp.handlers);
				lp.handlers = [];
			}
	...
	return this.sequence_compile(data);
}
...
Parser.prototype.read_bnf_sequence_ = function(str,pos) {
	var data = seq({
		...
		patterns: rep(seq({
				pattern: any( // паттерны
					bnf_symbol.then((symbol,x)=>({type:'symbol',symbol,pos:x})),
					comment.then((comment,x)=>({type:'comment',pos:x})),
					seq({
						none1: txt('('),
						none2: spcs,
		...
		hendlers: rep(seq(need(0),this.handler.then(pos_adder),spcs))
	}).then(
		(data)=>{
			if(!data.separator && data.patterns.length>0) {
				let lp = data.patterns[data.patterns.length-1];
